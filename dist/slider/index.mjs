...
}, { styled: w, css: gr, keyframes: yr, createTheme: xr } = tt({
  theme: ct,
  media: ut
}), pt = w("div", {
  display: "flex",
...
}, It = (t, o) => Object.hasOwn(t, o) ? t[o].toString() : void 0, Lt = w("span", {}), $e = (t) => {
  ...
  (function(l, f) {
    ...
    class q {
      ...
      traverseImageResource(e) {
        ...
        for (const b of u)
          typeof b == "string" ? x.push(this.traverseContentResource({ "@id": b, "@type": "dctypes:Image" })) : x.push(this.traverseContentResource(b));
      ...
      traverseType(e, i) {
        return i.reduce((u, x) => {
          const b = x(u);
          return b !== void 0 || this.options.allowUndefinedReturn ? b : u;
        }, e);
      }
    }
    ...
    function ue(r) {
      ...
      if (i) {
        ...
        for (const b of ["sc", "oa", "dcterms", "dctypes", "iiif"])
          if (i.startsWith(`${b}:`)) {
            i = i.slice(b.length + 1);
            break;
          }
        switch (i) {
          case "Layer":
    ...
    function W(r) {
      const [e, i] = function(b, $ = "Rights/License", ne = "none") {
        let ie = null;
        const Ce = [], Ze = Array.isArray(b) ? b : [b];
        for (const Ee of Ze) {
        ...
        return [ie, Ce];
      }(r.license), u = [...r.metadata ? (x = r.metadata, x ? x.map((b) => ({ label: k(b.label), value: k(b.value) })) : []) : [], ...i];
      var x;
    ...
    const Ke = new q({ collection: [function(r) {
      ...
      const { "@id": e, "@type": i, "@context": u, profile: x, ...b } = r, $ = {};
      return e && ($["@id"] = e), $["@type"] = ue(r), $["@type"] === "unknown" && (u && u.length && ($["@context"] = u), $["@type"] = "Service"), x && ($.profile = re(x)), B({ ...$, ...b });
    }], layer: [function(r) {
